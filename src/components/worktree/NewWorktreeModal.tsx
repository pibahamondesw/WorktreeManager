...
export function NewWorktreeModal({ open, onClose, repo, onCreated, editorApp }: NewWorktreeModalProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const worktreePath = `${repo.worktreeBasePath}/${selected.branchName}`;

      // 1. Fetch latest & create git worktree from origin/main
      ...
      // 2. Mark issue as started in Linear
      setCreatingStatus("Updating Linear issue...");
      await linear?.startIssue(selected.id);

      // 3. Save worktree record BEFORE opening Cursor
      //    so a Cursor failure doesn't lose the worktree
      ...
      // 4. Open editor (best-effort: failure here is non-fatal)
  ...
  return (
    <Modal open={open} onClose={onClose} title="New Worktree" wide>
